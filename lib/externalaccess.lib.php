			...
			for ($i=-7; $i<15; $i++){
				...
				for ($j=1; $j<3; $j++){
					if ($j==1){
						...
						$infobulle="";
						$path="";
						//afficher les dates pr�visionnelles
						...
						print ' <td ';
						//afficher les statuts atelier de l'apr�s midi
						$bgcolor='';
						if($planning[$object->immatriculation][$date]["apr�s-midi"]['color']){
							$bgcolor= 'bgcolor="'.$planning[$object->immatriculation][$date]["apr�s-midi"]['color'].'"';
						}
...
function calculPlanning($socId, $dateDebut, $dateFin){
	...
	//r�cup�ration de la couleur de l'�tat quand le vehicule est � l'atelier
	...
	while ($object = $db->fetch_object($resql)){		
		...
		while ($plannedTime<$endTime){			
			...
			if ($heure>0 && $heure<1201){
				...
				$planning[$object->immatriculation][$jour]["apr�s-midi"]['color']=$object->color;
			}			
			$i+=3600;
			$plannedTime+=$i;
		}	
	...
	//r�cup�ration des dates pr�visionnelles de maintenance � programmer

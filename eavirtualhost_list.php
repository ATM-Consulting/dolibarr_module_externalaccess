...
/**
 *   	\file       eavirtualhost_list.php
 *		\ingroup    externalaccess
 *		\brief      List page for eavirtualhost
 */
...
$hookmanager->initHooks(array('eavirtualhostlist')); // Note that conf->hooks_modules contains array
...
if (!isModEnabled("externalaccess")) {
	accessforbidden('Module externalaccess not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'EAVirtualHost';
	$objectlabel = 'EAVirtualHost';
	$uploaddir = $conf->externalaccess->dir_output;
...
//$help_url = "EN:Module_EAVirtualHost|FR:Module_EAVirtualHost_FR|ES:Módulo_EAVirtualHost";
$help_url = '';
$title = $langs->trans("EAVirtualHosts");
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalString('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/externalaccess/eavirtualhost_card.php', 1).'?id='.$id);
	exit;
}
...
require_once __DIR__ . '/lib/externalaccess.lib.php';
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/externalaccess/eavirtualhost_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendEAVirtualHostRef";
$modelmail = "eavirtualhost";
$objecttmp = new EAVirtualHost($db);
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_externalaccess', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
